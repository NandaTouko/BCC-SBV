    ...
    for(int i = 0; i < LINHAS; i++){
        for(int j = 0; j < COLUNAS; j++){
            ...
            switch (saida[i][j]) {
                ...
                case ' ': c = WHITE; break;
                case 'r': 
                    DrawLine((i * largQuadrado), (j * largQuadrado)/2, largQuadrado * (i+1), (j * largQuadrado)/2, GREEN);
                    // c = GREEN; 
                    break;
                case 'd': c = YELLOW; break;
                case 'l': c = RED; break;
                case 'u': c = ORANGE; break;
                case 's': c = BLUE; break;
                case 't': c = DARKBLUE; break;
                default:  c = WHITE; break;
            }

            DrawRectangle(largQuadrado * j, largQuadrado * i, largQuadrado, largQuadrado, c);
        }
    }
...
void createGameWorld( void ) {
    ...
    temSolucao = resolver(linhaFonte, colunaFonte, 4, 5);
    saida[linhaFonte][colunaFonte] = 's';
...
bool resolver(int linhaFonte, int colunaFonte, int linhaAlvo, int colunaAlvo) {
    if(ehValido(linhaFonte, colunaFonte)){
        ...
        if(linhaFonte == linhaAlvo && colunaFonte == colunaAlvo) {
            saida[linhaAlvo][colunaAlvo] = 't'; // t -> target (alvo)
            return true;
        }

        // direita
        if(resolver(linhaFonte, colunaFonte + 1, linhaAlvo, colunaAlvo)) {
            saida[linhaFonte][colunaFonte] = 'r'; // r -> right
            return true;
        }

        // baixo
        if(resolver(linhaFonte + 1, colunaFonte, linhaAlvo, colunaAlvo)) {
            saida[linhaFonte][colunaFonte] = 'd'; // d -> down
            return true;
        }

        // esquerda
        if(resolver(linhaFonte, colunaFonte - 1, linhaAlvo, colunaAlvo)) {
            saida[linhaFonte][colunaFonte] = 'l'; // l -> left
            return true;
        }

        // cima
        if(resolver(linhaFonte - 1, colunaFonte, linhaAlvo, colunaAlvo)) {
            saida[linhaFonte][colunaFonte] = 'u'; // u -> up
            return true;
        }
    }
